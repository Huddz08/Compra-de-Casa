...
function receiveState(data) {
  const working = editing && dirty && current() ? structuredClone(current()) : null;
  ...
  if (editing && !dirty) draftBase = current() ? structuredClone(current()) : null;
  render();
}
function openList(id) {
  selected = id; tab = 'list'; filter = ''; state = structuredClone(remoteState);
  editing = Boolean(current()); draftBase = current() ? structuredClone(current()) : null; dirty = false; render(); window.scrollTo({top:0});
}
async function saveEditor(finish = null) {
  ...
  const ok = await persist(next => {
    ...
    const saved = merged.lists.find(l => l.id === draft.id);
    if (finish) { Object.assign(saved, finish); finalizeList(merged, draft.id, new Date().toISOString(), finish.actualTotal); }
    else validateListDetails(saved);
...
function navigate(action) {
  ...
  const proceed = () => { editing = false; dirty = false; draftBase = null; state = structuredClone(remoteState); action(); render(); window.scrollTo({top:0}); };
...
$('#import-backup').onclick = () => {
  ...
  $('#confirm-import').onclick = async () => { if (!imported) return; if (await persist(s=>{if(s.products.length||s.lists.length)throw new Error('A casa já recebeu dados. Importação cancelada.');s.products=imported.products;s.lists=imported.lists;})) { selected='';closeModal();render();toast('Backup importado.'); } };
};
...
function renderList() {
  const l = current();
  if (!l) return renderListLibrary();
  const pending = pendingItems(l).length;
  $('#content').innerHTML = title('LISTA DA CASA',esc(l.name),`${esc(l.market || 'Mercado a definir')} · ${displayDate(l.purchaseDate)}`,'<button class="text-button" id="back-lists">← Listas</button>') + `
    <div class="editor-meta"><span>${esc(l.market || 'Mercado a definir')} · ${displayDate(l.purchaseDate)} · ${l.closed?'Concluída':'Em aberto'}</span><button id="edit-details">Editar dados</button></div>
    <div class="stats shopping-stats"><article class="stat green"><span>${l.closed?'TOTAL PAGO':'TOTAL ESTIMADO'}</span><strong>${money(l.closed?paidTotal(l):total(l))}</strong></article><article class="stat"><span>${l.closed?'VALOR DOS ITENS':'NO CARRINHO'}</span><strong>${money(total(l,true))}</strong></article><article class="stat"><span>${l.closed?'COMPRADOS':'PROGRESSO'}</span><strong>${l.items.filter(i=>i.checked).length}<em> / ${l.items.length}</em></strong></article></div>
    <p class="compact-price-note">${l.items.filter(i=>i.price==null).length} itens sem preço · ${l.closed?'Valor do caixa registrado separadamente.':'Preços anteriores são estimativas até você conferir.'}</p>
    ${l.closed?`<div class="checkout-summary">${adjustmentText(checkoutAdjustment(l))}</div>`:''}
    ${pending?`<div class="pending-banner">${pending} ${pending===1?'item não comprado':'itens não comprados'}${l.followupListId?' nesta compra.':''} ${l.followupListId?'<button id="open-followup">Ver lista de pendências →</button>':'· ainda falta colocar no carrinho.'}</div>`:''}
    ${l.sourceListId?'<button class="source-link" id="open-source">← Ver compra de origem</button>':''}
    <section class="list-card"><div class="card-toolbar"><h2>Itens <span>${l.items.length}</span></h2><button id="add-item" class="primary" ${l.closed?'disabled':''}>＋ Adicionar item</button></div><div class="search-row"><span>⌕</span><input id="search" placeholder="Buscar na lista…" aria-label="Buscar na lista" value="${esc(filter)}"><small>VALORES POR EMBALAGEM</small></div><div class="items-heading" aria-hidden="true"><span>✓</span><span>Produto</span><span>Qtd.</span><span>Preço R$</span><span>Total R$</span><span></span></div><div id="items"></div></section>
    <div class="editor-save"><span>${dirty?'● Alterações não salvas':'✓ Lista salva'}</span><button id="save-list" class="primary" ${dirty?'':'disabled'}>Salvar lista</button>${!l.closed?'<button id="close-list" class="primary">Finalizar compra</button>':''}</div>`;
  $('#back-lists').onclick=()=>navigate(()=>{selected='';});
  $('#edit-details').onclick=()=>listDetails();
  $('#save-list').onclick=()=>saveEditor();
  $('#search').oninput=e=>{filter=e.target.value;renderItems();};
  $('#add-item').onclick=()=>itemForm();
  if($('#close-list'))$('#close-list').onclick=checkoutForm;
  if($('#open-followup'))$('#open-followup').onclick=()=>navigate(()=>openList(l.followupListId));
  if($('#open-source'))$('#open-source').onclick=()=>navigate(()=>openList(l.sourceListId));
  renderItems();
}
function adjustmentText(value) {
  return value===0?'Sem diferença no caixa':`${value<0?'Desconto':'Acréscimo'} no caixa: ${money(Math.abs(value))}`;
}
function renderListLibrary() {
  const lists=[...state.lists].sort((a,b)=>Number(a.closed)-Number(b.closed)||b.month.localeCompare(a.month)||b.createdAt.localeCompare(a.createdAt));
  $('#content').innerHTML=title('ORGANIZAÇÃO DA CASA','Listas de compras','Escolha a lista que vai levar ao mercado.','<button id="new-list" class="primary">＋ Nova lista</button>')+`<div class="lists-grid">${lists.length?lists.map(l=>`<article class="list-tile"><span class="pill">${l.closed?'Concluída':l.sourceListId?'Pendências':'Em aberto'}</span><h2>${esc(l.name)}</h2><p>${esc(l.market||'Mercado a definir')}<br>${l.purchaseDate?displayDate(l.purchaseDate):monthName(l.month)}</p><strong>${money(l.closed?paidTotal(l):total(l))}</strong><small>${l.items.filter(i=>i.checked).length} de ${l.items.length} itens comprados${pendingItems(l).length?' · '+pendingItems(l).length+(l.followupListId?' transferidos':' pendentes'):''}</small><button class="primary" data-open-list="${l.id}">Abrir lista →</button></article>`).join(''):'<section class="empty welcome"><h2>Sua próxima compra começa aqui</h2><p>Crie uma lista do zero ou copie uma compra anterior.</p></section>'}</div>`;
  $('#new-list').onclick=newList;
  document.querySelectorAll('[data-open-list]').forEach(b=>b.onclick=()=>openList(b.dataset.openList));
}
function listFields(l) {
  return `<label>Nome da lista<input name="name" required maxlength="70" value="${esc(l.name||'')}" placeholder="Compra do mês"></label><div class="form-grid"><label>Mês de referência<input name="month" type="month" required value="${l.month||nowMonth()}"></label><label>Data da compra<input name="purchaseDate" type="date" value="${l.purchaseDate||''}"></label></div><label>Mercado<input name="market" maxlength="100" list="known-markets" value="${esc(l.market||'')}" placeholder="Ex.: Mercado do Bairro"></label><datalist id="known-markets">${[...new Set(state.lists.map(x=>x.market).filter(Boolean))].map(m=>`<option value="${esc(m)}"></option>`).join('')}</datalist><label>Observações<textarea name="notes" maxlength="500" rows="2">${esc(l.notes||'')}</textarea></label>`;
}
function readListFields(form) {
  return {name:form.get('name').trim(),month:form.get('month'),purchaseDate:form.get('purchaseDate'),market:form.get('market').trim(),notes:form.get('notes').trim()};
}
...
function checkoutForm() {
  ...
  if(!bought.length)return toast('Marque pelo menos um item no carrinho.');
  if(bought.some(i=>i.price==null))return toast('Informe o preço de todos os itens no carrinho.');
  modal('Conferir e finalizar compra',`<form id="checkout-form"><p>Somatório de ${bought.length} itens no carrinho: <strong>${money(subtotal)}</strong>.</p><label>Valor real pago no caixa (R$)<input id="checkout-total" name="actualTotal" type="number" min="0" max="9999999" step="0.01" required value="${subtotal.toFixed(2)}"></label><p id="checkout-difference" role="status">Sem diferença no caixa</p><label>Mercado<input name="market" required maxlength="100" value="${esc(l.market||'')}" placeholder="Onde você está comprando?"></label><label>Data da compra<input name="purchaseDate" type="date" required value="${l.purchaseDate||today()}"></label><p>${pending?`${pending} itens não comprados irão automaticamente para uma nova lista de pendências, para você comprar em outro mercado.`:'Todos os itens estão no carrinho.'}</p><p class="form-note">Finalizar salva a lista e registra o total pago. Desconto ou acréscimo geral não altera os preços individuais no histórico.</p><button class="primary">Salvar e finalizar compra</button></form>`);
  $('#checkout-total').oninput=e=>{$('#checkout-difference').textContent=e.target.value===''?'Informe o total do caixa.':adjustmentText(roundMoney(Number(e.target.value)-subtotal));};
  $('#checkout-form').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);if(await saveEditor({actualTotal:Number(f.get('actualTotal')),market:f.get('market').trim(),purchaseDate:f.get('purchaseDate')}))closeModal();};
}
...
function renderItems() {
  ...
  document.querySelectorAll('[data-item]').forEach(el=>el.onclick=()=>l.closed?productHistory(l.items.find(i=>i.id===el.dataset.item).productId):priceForm(el.dataset.item));
  ...
  document.querySelectorAll('[data-remove]').forEach(el=>el.onclick=()=>{modal('Remover item?', '<p>O produto continuará no cadastro e nas compras anteriores.</p><button class="primary" id="confirm-remove">Remover desta lista</button>');$('#confirm-remove').onclick=async()=>{if(await mutate(s=>{const list=s.lists.find(x=>x.id===l.id);list.items=list.items.filter(i=>i.id!==el.dataset.remove);}))closeModal();};});
}
function newList() {
  modal('Nova lista de compras', '<form id="new-list-form">'+listFields({month:nowMonth()})+'<label>Como começar?<select name="source"><option value="">Criar do zero</option>'+[...state.lists].sort((a,b)=>b.month.localeCompare(a.month)).map(l=>'<option value="'+l.id+'">Copiar '+esc(l.name)+'</option>').join('')+'</select></label><p class="form-note">Os preços vêm das compras anteriores do mesmo produto. A lista copiada começa sem itens marcados.</p><button class="primary">Criar e abrir lista</button></form>');
  $('#new-list-form').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target), id=uid();if(await persist(s=>{const l={id,...readListFields(f),createdAt:new Date().toISOString(),closed:false,items:[]};validateListDetails(l);const source=s.lists.find(x=>x.id===f.get('source'));if(source)source.items.forEach(i=>addItem(s,l,i.productId,i.quantity));s.lists.push(l);})){closeModal();openList(id);}};
}
function itemForm(catalogOnly=false) {
  ...
  $('#item-form').onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);if(await mutate(s=>{let pid=f.get('existing');if(!pid){const p={id:uid(),name:f.get('name').trim(),brand:f.get('brand').trim(),type:f.get('type').trim(),size:Number(f.get('size')),unit:f.get('unit'),category:f.get('category')};if(!p.name||!p.brand||!p.type)throw new Error('Preencha nome, marca e tipo.');const existing=s.products.find(x=>productKey(x)===productKey(p));pid=existing?.id||p.id;if(!existing)s.products.push(p);else if(catalogOnly)throw new Error('Este produto já está cadastrado.');}if(!catalogOnly){const list=s.lists.find(x=>x.id===l.id);if(list.closed)throw new Error('Esta compra já foi concluída.');addItem(s,list,pid,Number(f.get('quantity')));}}))closeModal();};
}
function priceForm(id, photo=false) {
  const l=current(), item=l.items.find(i=>i.id===id), p=product(item.productId);
  const history = purchaseHistory(state, p.id), best = history.reduce((a,b)=>!a||b.price<a.price?b:a,null);
  modal('Item da lista',`<p><strong>${esc(p.name)}</strong><br>${esc(description(p))}</p>${best?`<p class="best-price">Menor preço registrado: <strong>${money(best.price)}</strong> em ${esc(best.market||'mercado não registrado')} · ${displayDate(best.date)}</p>`:''}<div class="photo-box"><label class="upload-label">▣ Fotografar ou escolher etiqueta<input id="photo-input" type="file" accept="image/*" capture="environment"></label><small>Aproxime a câmera do preço por embalagem.</small><img id="photo-preview" alt="Etiqueta escolhida" hidden><p id="ocr-status" role="status"></p><div id="candidates"></div></div><form id="price-form"><label>Preço por embalagem (R$)<input id="price-value" name="price" type="number" inputmode="decimal" min="0.01" max="99999" step="0.01" required value="${item.price??''}"></label><label class="cart-check"><input type="checkbox" name="inCart" ${item.checked?'checked':''}> Já coloquei no carrinho</label><p class="form-note">Confirme se é o preço correto para esta embalagem e condição de compra. A foto é processada no aparelho e não fica salva.</p><button class="primary">Aplicar preço à lista</button></form>`);
  $('#price-form').onsubmit=async e=>{e.preventDefault();const values=new FormData(e.target),price=Number(values.get('price')); if(await mutate(s=>{const list=s.lists.find(x=>x.id===l.id);if(list.closed)throw new Error('Esta compra já foi concluída.');const i=list.items.find(i=>i.id===id);i.price=price;i.updated=true;i.checked=values.get('inCart')==='on';}))closeModal();};
  ...
  let reading = false, cancelled = false, photoURL;
  modalCleanup = () => { cancelled = true; if(photoURL)URL.revokeObjectURL(photoURL); photoURL=null; input.value=''; preview.removeAttribute('src'); }; 
  ...
  input.onchange = async () => {
    ...
    const url = URL.createObjectURL(file); photoURL = url;
    try {
      ...
      if (found) {
        ...
        output.textContent = 'Preço preenchido: ' + money(prices[0]) + (prices.length > 1 ? '. Há outros valores na etiqueta: confira o sugerido ou toque em uma alternativa.' : '. Confira e aplique o preço à lista.');
        if (prices.length > 1) {
    ...
    finally { URL.revokeObjectURL(url); photoURL=null; reading = false; input.disabled = false; input.value = ''; value.disabled = false; submit.disabled = false; }
  };
  if(photo)input.click();
}
function renderCatalog() {
  $('#content').innerHTML=title('CADA PRODUTO, DO SEU JEITO','Nossos produtos','Toque em um produto para ver preços, datas e mercados.','<button id="new-product" class="primary">＋ Cadastrar produto</button>')+`<section class="catalog-grid">${state.products.length?state.products.map(p=>{const history=purchaseHistory(state,p.id),last=history.at(-1);return `<button class="product-card history-card" data-product-history="${p.id}"><span class="category">${esc(p.category)}</span><h2>${esc(p.name)}</h2><p>${esc(description(p))}</p><strong>${last?money(last.price):'Sem compras registradas'}</strong><small>${last?esc(last.market||'Mercado não registrado')+' · '+displayDate(last.date):'O histórico começa ao finalizar uma compra.'}</small><span class="history-link">Ver evolução do preço →</span></button>`;}).join(''):'<div class="empty"><h2>O começo de um bom planejamento</h2><p>Cadastre seus produtos aqui ou ao adicionar itens à lista.</p></div>'}</section>`;
  $('#new-product').onclick=()=>itemForm(true);
  document.querySelectorAll('[data-product-history]').forEach(b=>b.onclick=()=>productHistory(b.dataset.productHistory));
}
function productHistory(id) {
  const p=product(id), history=purchaseHistory(state,id);
  if(!history.length){modal('Histórico de '+p.name,`<p>${esc(description(p))}</p><p>Ainda não há compras concluídas para este produto.</p>`);return;}
  const min=Math.min(...history.map(h=>h.price)),max=Math.max(...history.map(h=>h.price));
  const low=Math.max(0,min-(max-min||min||1)*.15),high=max+(max-min||max||1)*.15;
  const points=history.map((h,i)=>({x:history.length===1?250:55+i/(history.length-1)*415,y:185-(h.price-low)/(high-low)*155}));
  const best=history.reduce((a,b)=>b.price<a.price?b:a);
  modal('Histórico de '+p.name,`<p>${esc(description(p))}</p><p class="best-price">Menor preço registrado: <strong>${money(best.price)}</strong><br>${esc(best.market||'Mercado não registrado')} · ${displayDate(best.date)}</p><p class="form-note">Cada ponto representa uma compra, em ordem de data. Toque para ver os detalhes.</p><svg class="price-history" viewBox="0 0 500 230" aria-label="Evolução do preço por compra"><line x1="50" y1="190" x2="480" y2="190" stroke="#dce5d8"/><text x="3" y="30">${compactMoney(high)}</text><text x="3" y="185">${compactMoney(low)}</text><polyline points="${points.map(p=>`${p.x},${p.y}`).join(' ')}" fill="none" stroke="#17614d" stroke-width="3"/>${points.map((point,i)=>`<g role="button" tabindex="0" class="history-point" data-point="${i}" aria-label="Compra ${i+1}: ${money(history[i].price)}, ${displayDate(history[i].date)}, ${esc(history[i].market||'mercado não registrado')}"><circle cx="${point.x}" cy="${point.y}" r="18" fill="transparent"/><circle class="point-dot" cx="${point.x}" cy="${point.y}" r="6" fill="#17614d" stroke="white" stroke-width="2"/></g>`).join('')}<text x="50" y="218">Compra 1</text><text x="410" y="218">Compra ${history.length}</text></svg><div id="history-detail" aria-live="polite"></div><div class="history-purchases">${history.map((h,i)=>`<button data-history-index="${i}">${h.date?displayDate(h.date):monthName(h.month)+' (sem dia)'} · ${money(h.price)}</button>`).join('')}</div><p class="form-note">Preço por embalagem. Ajustes gerais do caixa entram no gasto total da compra e não são distribuídos entre os produtos.</p>`);
  const choose=index=>{
    const h=history[index];
    $('#history-detail').innerHTML=`<strong>${money(h.price)} por embalagem</strong><dl><dt>Data da compra</dt><dd>${displayDate(h.date)}</dd><dt>Lista</dt><dd>${esc(h.listName)}</dd><dt>Mercado</dt><dd>${esc(h.market||'Não registrado')}</dd><dt>Quantidade</dt><dd>${h.quantity}</dd></dl><button id="history-open-list" class="text-button">Abrir esta lista →</button>`;
    document.querySelectorAll('[data-point]').forEach(el=>{el.setAttribute('aria-pressed',String(Number(el.dataset.point)===index));el.querySelector('.point-dot').setAttribute('r',Number(el.dataset.point)===index?'9':'6');});
    document.querySelectorAll('[data-history-index]').forEach(el=>el.setAttribute('aria-pressed',String(Number(el.dataset.historyIndex)===index)));
    $('#history-open-list').onclick=()=>{closeModal();navigate(()=>openList(h.listId));};
  };
  document.querySelectorAll('[data-point]').forEach(el=>{el.onclick=()=>choose(Number(el.dataset.point));el.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();choose(Number(el.dataset.point));}};});
  document.querySelectorAll('[data-history-index]').forEach(el=>el.onclick=()=>choose(Number(el.dataset.historyIndex)));
  choose(history.length-1);
}

function renderAnalytics() {
  const months=monthlySpend(state), latest=months.at(-1), prev=latest?months.find(([m])=>m===previousMonth(latest[0])):null;
  $('#content').innerHTML=title('OLHAR PARA OS NÚMEROS, CUIDAR DA CASA','Evolução dos gastos','Um retrato das compras concluídas, mês a mês.')+(!latest?'<section class="empty welcome"><span class="empty-icon">▥</span><h2>Seu histórico começa na primeira compra</h2><p>Finalize uma lista para ver os gastos e acompanhar os preços.</p></section>':`<div class="stats"><article class="stat green"><span>ÚLTIMO MÊS · ${monthName(latest[0]).toUpperCase()}</span><strong>${money(latest[1])}</strong><small>Inclui ajustes do caixa</small></article><article class="stat"><span>EM RELAÇÃO AO MÊS ANTERIOR</span><strong>${prev?`${latest[1]>=prev[1]?'＋':'−'}${money(Math.abs(latest[1]-prev[1]))}`:'—'}</strong><small>${prev&&prev[1]>0?`${((latest[1]/prev[1]-1)*100).toLocaleString('pt-BR',{minimumFractionDigits:1,maximumFractionDigits:1})}% de variação no gasto`:'Sem base no mês imediatamente anterior'}</small></article><article class="stat"><span>COMPRAS CONCLUÍDAS</span><strong>${state.lists.filter(l=>l.closed).length}</strong><small>Em ${months.length} meses com registros</small></article></div><section class="chart-card"><h2>Quanto a casa gastou</h2><p>Gasto total mensal · R$</p>${barChart(months)}</section><section class="chart-card"><h2>Comparar dois meses</h2><div class="form-grid"><label>Primeiro mês<select id="compare-a">${months.map(([m])=>`<option value="${m}" ${m===(prev?.[0]||months[0][0])?'selected':''}>${monthName(m)}</option>`).join('')}</select></label><label>Segundo mês<select id="compare-b">${months.map(([m])=>`<option value="${m}" ${m===latest[0]?'selected':''}>${monthName(m)}</option>`).join('')}</select></label></div><div id="comparison"></div></section><section class="chart-card"><h2>O preço de cada produto</h2><label>Produto<select id="chart-product">${state.products.map(p=>`<option value="${p.id}">${esc(p.name+' · '+description(p))}</option>`).join('')}</select></label><div id="product-chart"></div></section>`);
  if(!latest)return;
  const compare=()=>{const a=$('#compare-a').value,b=$('#compare-b').value;const ta=months.find(([m])=>m===a)[1],tb=months.find(([m])=>m===b)[1];const get=m=>{const map=new Map();state.lists.filter(l=>l.closed&&spendMonth(l)===m).forEach(l=>l.items.filter(i=>i.checked&&i.price!=null).forEach(i=>{const v=map.get(i.productId)||{q:0,sum:0};v.q+=i.quantity;v.sum+=i.quantity*i.price;map.set(i.productId,v);}));return map;};const ma=get(a),mb=get(b);const ids=[...new Set([...ma.keys(),...mb.keys()])];$('#comparison').innerHTML=`<div class="comparison-summary">${monthName(a)}: <strong>${money(ta)}</strong> <span>→</span> ${monthName(b)}: <strong>${money(tb)}</strong><br><small>Diferença no gasto: ${tb>=ta?'+':'−'}${money(Math.abs(tb-ta))}. Inclui quantidades, produtos e ajustes do caixa.</small></div><div class="table-scroll"><table><thead><tr><th>Produto / embalagem</th><th>Preço médio · 1º mês</th><th>Preço médio · 2º mês</th><th>Variação unitária</th></tr></thead><tbody>${ids.map(id=>{const p=product(id),x=ma.get(id),y=mb.get(id);return `<tr><td><strong>${esc(p.name)}</strong><small>${esc(description(p))}</small></td><td>${x?money(x.sum/x.q):'Não comprado'}</td><td>${y?money(y.sum/y.q):'Não comprado'}</td><td>${x&&y?delta(y.sum/y.q,x.sum/x.q):'Sem par comparável'}</td></tr>`;}).join('')}</tbody></table></div><p class="form-note">Preço médio por embalagem, ponderado pela quantidade comprada. Apenas cadastros idênticos são comparados.</p>`;};
...
window.addEventListener('storage', e=>{if(!cloud&&e.key==='compra-de-casa-v1'&&e.newValue){try{receiveState(JSON.parse(e.newValue));}catch{toast('Erro ao receber dados de outra aba.');}}});
